...
| `use_urdfs` | bool | Optional | Selects which kinematics representation `get_kinematics` serves: `true` serves the model's URDF file, `false` serves the SVA JSON file. When `true` and the model ships no URDF, the request errors. **Default false** |
| `max_tcp_speed_m_per_sec` | float | Optional | Default cap on tool-center-point (Cartesian) speed in meters per second, applied to every `MoveThroughJointPositions` call. A per-call `MoveOptions.max_tcp_speed` overrides this value. Requires `enable_new_trajectory_planner` true and a model with a URDF. (range: > 0) |
